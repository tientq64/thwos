await task.import [
	"npm:codemirror@5.53.2/lib/codemirror.min.css"
	"npm:codemirror@5.53.2"
	"npm:codemirror@5.53.2/keymap/sublime.min.js"
]
...
class App extends React.Component
	constructor: (props) ->
		...
		,
			...
			menu: [
				text: "Hoàn tác"
				label: "Ctrl+Z"
				disabled: => not @tab?.cm.historySize().undo
				onClick: =>
					@tab?.cm.undo()
					return
			,
				text: "Làm lại"
				label: "Ctrl+Y"
				disabled: => not @tab?.cm.historySize().redo
				onClick: =>
					@tab?.cm.redo()
					return
			,,
		...
		,
			...
			,
				...
				submenu: @themes.map (theme) =>
					text: theme
					icon: => "dot" if theme is @config.theme
					onClick: =>
						@setTheme theme
						return
			]
	...
	setOption: (key, val) ->
		@config[key] = val
		for tab from @tabs
			tab.cm?.setOption key, val
		@setState {}
		return

	setTheme: (theme) ->
		await task.import "npm:codemirror@5.53.2/theme/#{theme}.min.css"
		@setOption "theme", theme
		@updateStyle()
		return

	setAddon: (addon) ->
		await task.import "npm:codemirror@5.53.2/addon/#{addon}.min.js"
	...
	loadMode: (mode) ->
		...
		if mode.name
			await task.import "npm:codemirror@5.53.2/mode/#{mode.name}/#{mode.name}.min.js"
		return
	...
	newTab: (entry) ->
		new Promise (resolve) =>
			((tab) =>
				tab =
					id: "CodeEditor__tabPanelId#{_.uniqueId()}"
					entry: entry
					mode: @modes.find (mode) => mode.text is "Plain Text"
					cm: null
				@tabs.push tab
				@setState {}, =>
					tab.cm = CodeMirror.fromTextArea @refs[tab.id], @config
					if entry
		...
		switch name
			when "$entries"
				if val.length
					for entry from val
						await @newTab entry
				if @isFirstListenEntries
					@isFirstListenEntries = no
					unless val.length
						@newTab()
		return
	...
	render: ->
		<div className="column full bg-dark-gray2 text-white">
			<div className="col-0">
				<Menubar
					className="bg-dark-gray3 text-white"
					menu={@menubar}
					onClosing={@onClosingMenubar}
				/>
			</div>
