...
class App extends React.Component
	...
	handleContextMenuEntry: (entry, event) ->
		...
		,
			...
			onClick: =>
				await ap.setDesktopBackgroundBase64 entry.fullPath
				@setState {}
				return
		,,
			text: "Chia sẻ"
			icon: "share"
			submenu: [
				text: "Chia sẻ qua FileIO..."
				icon: "share"
				onClick: =>
					ap.runTask "/C/programs/FileIO/app.yml", null, entries: @selEntries
					return
			]
		,,
			text: "Di chuyển"
	...
	onContextMenuList: (event) ->
		...
		,,
			...
			onClick: =>
				...
				return
		,
			text: "Mở cửa sổ dòng lệnh tại đây..."
			icon: "console"
			onClick: =>
	...
	render: ->
		if @entry
			...
			<div
				className="column full p-3 no-scroll FileManager__main #{@classMain()}"
				style={@styleMain()}
			>
				{unless task.env.isDesktop
					<form className="col-0 row" onSubmit={@onSubmitForm}>
				...
				{if task.env.picker
					<div className="col-0 row middle mt-3">
