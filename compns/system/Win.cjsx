class Win extends React.Component
	constructor: (props) ->
		...
		width = minWidth if width is "auto"
		...
		@sandbox = sandbox
		@isAutoWidth = task.env.width is "auto"
		@isAutoHeight = task.env.height is "auto"
		@isAutoSize = @isAutoWidth or @isAutoHeight
	...
	close: (resolveVal) ->
		if @task.media.stream
			for track from @task.media.stream.getTracks()
				track.stop()
			@task.media.video.pause()
			@task.media.video.srcObject = null
			@task.media.video.remove()
			@task.media.g.canvas.remove()
			cancelAnimationFrame @task.media.raf
	...
	requestPerm: (name, ...val) ->
		switch name
			when "paths"
				...
				path = await ap.resolveShortcut path
				if @task.appl.perms.paths.some (v) => app.pathInPath path, v then path
	...
	runTask: (filePath, params, env) ->
		filePath = ap.pathJoin @task.appl.path, filePath
		app.runTask filePath, params, env, @task

	runPicker: (filePath, params, env) ->
		filePath = ap.pathJoin @task.appl.path, filePath
		app.runPicker filePath, params, env, @task
	...
	iframeDidResize: (width, height) ->
		if @isAutoSize
			if @isAutoWidth
				width = _.clamp width + 2, @minWidth, @maxWidth
				@x -= (width - @width) // 2
				@x = _.clamp @x, 0, app.desktop.width - width
				@width = width
			if @isAutoHeight
				height = _.clamp height + 38, @minHeight, @maxHeight
				@y -= (height - @height) // 2
		...
		if @task.env.resizable
			switch event.type
				when "panstart"
					...
					@isAutoSize = no
					@animMinimize = null
					app.setCursorTouchmove @resizeEdge.replace("-", "") + "-resize"
				when "pan"
					if event.isFinal
