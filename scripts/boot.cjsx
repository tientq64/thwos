{
	CSSTransition, ReplaceTransition, SwitchTransition, Transition, TransitionGroup
...
React.Component::$autoBind = (target = @, ctor = target.constructor) ->
	...
	return
React.Component::$onChange = (target, prop, cb) ->
	if arguments.length is 2 and typeof prop is "function" or arguments.length is 1
		[target, prop, cb] = [@, target, prop]
	(event) =>
		_.set target, prop, event.target.value
		cb? prop, event
		@setState {}
		return
React.Component::$model = (target, prop, cb) ->
	if arguments.length is 2 and typeof prop is "function" or arguments.length is 1
		[target, prop, cb] = [@, target, prop]
	value: _.get target, prop
	onChange: @$onChange target, prop, cb
